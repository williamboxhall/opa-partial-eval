...
class OpaPartialEvalTest {
    ...
    fun `generates SQL fragments for all operator types`() {
        assertEquals(
            "((entity.a = 1 AND entity.b = ['bar', 'foo'] AND FALSE IN entity.c AND entity.d != 2 AND entity.e > 3 AND entity.i + 1 = 7 AND entity.i2 + 2 > 72 AND count(entity.f) = 4 AND count(entity.f2) > 42 AND count(entity.f3) + 3 = 43 AND count(entity.f4) + 4 < 44 AND abs(entity.g) = 5 AND abs(entity.g2) > 52 AND abs(entity.h) = 6 AND abs(entity.h2) > 62 AND entity.i2 + 3 + 4 + 5 = 72 AND entity.i3 + 3 + 4 + 5 > 73 AND entity.j % 8 = 8 AND entity.k = 0 AND max(entity.l) < 10 AND sort(entity.m) = ['a', 'b'] AND sum(entity.n) = 11 AND sum(entity.n2) + 1 = 12 AND starts_with(entity.o, \"foo\") = TRUE))",
